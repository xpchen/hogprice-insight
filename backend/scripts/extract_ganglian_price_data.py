...
class GanglianPriceExtractor:
    ...
    def extract_and_insert(self) -> Dict:
        ...
        results = {
            ...
            "frozen_capacity_rate": self._extract_frozen_capacity_rate(batch.id),
            "slaughter_weight": self._extract_slaughter_weight(batch.id),
            "monthly_data": self._extract_monthly_data(batch.id)
        }
    ...
    def _extract_frozen_capacity_rate(self, batch_id: int) -> Dict:
        """提取冻品库容率数据（从冻品库容率sheet）"""
        print("提取冻品库容率数据...")
        ...
        if df.empty:
            return {"error": "数据为空", "inserted": 0}
        
        # 第2列（index 1）是数据列
        if len(indicator_names) < 2:
            return {"error": "未找到指标列", "inserted": 0}
        
        raw_header = indicator_names[1] if len(indicator_names) > 1 else None
        if pd.isna(raw_header) or not raw_header:
            return {"error": "指标名称为空", "inserted": 0}
        
        raw_header_str = str(raw_header)
        unit = units[1] if len(units) > 1 else "百分比"
        
        # 获取或创建指标
        metric_key = "GL_W_FROZEN_CAPACITY_RATE"
        metric = self._get_or_create_metric(
            metric_key=metric_key,
            metric_name="冻品库容率",
            source_code="GANGLIAN",
        ...
        for row_idx, row in df.iterrows():
            try:
                ...
                value = clean_numeric_value(capacity_val)
                if value is None:
                    continue
                ...
                dedup_key = self._generate_dedup_key(
                    ...
                    geo_code="NATION",
                    obs_date=trade_date,
                    tags={"scope": "nation", "source": "GANGLIAN"}
                )
    ...
    def _extract_slaughter_weight(self, batch_id: int) -> Dict:
        """提取宰后均重数据（从宰后均重sheet，提取所有省区）"""
        print("提取宰后均重数据（所有省区）...")
        ...
        for col_idx in range(1, len(indicator_names)):
            ...
            # 获取或创建指标（每个省区一个指标）
            metric_key = f"GL_W_SLAUGHTER_WEIGHT_{province_name}"
            metric = self._get_or_create_metric(
                metric_key=metric_key,
                metric_name="宰后均重",
                source_code="GANGLIAN",
                sheet_name=sheet_name,
                unit=units[col_idx] if col_idx < len(units) else "千克",
                raw_header=raw_header_str,
                freq="W"  # 周度数据
            )
            ...
            for row_idx, row in df.iterrows():
                try:
                    ...
                    value = clean_numeric_value(weight_val)
                    if value is None:
                        continue
        ...
        log_entry = {
            "sheet": sheet_name,
            "metric": "宰后均重（分省区）",
            "metric_key": "GL_W_SLAUGHTER_WEIGHT",
            "inserted": total_inserted,
            "provinces": province_results,
            "date_range": {
                "start": str(df.iloc[0, 0]) if len(df) > 0 else None,
    ...
    def _extract_monthly_data(self, batch_id: int) -> Dict:
        """提取月度数据（从月度数据sheet，提取所有指标）"""
        print("提取月度数据（所有指标）...")
        ...
        for col_idx in range(1, len(indicator_names)):
            ...
            # 解析指标名称，格式："猪：出栏数：中国（月）"
            # 提取指标类型和区域
            import re
            parts = raw_header_str.split('：')
            if len(parts) < 3:
                print(f"跳过列 {col_idx}: 无法解析指标名称 - {raw_header_str}")
                continue
            
            # 提取指标类型（如"出栏数"、"存栏数"等）
            indicator_type = parts[1].strip() if len(parts) > 1 else ""
            # 提取区域（移除括号内容）
            region_part = parts[-1].strip() if len(parts) > 2 else ""
            region_name = re.sub(r'[（(].*?[)）]', '', region_part).strip()
            
            # 根据指标类型生成metric_key
            metric_key_map = {
                "出栏数": "GL_M_OUTPUT_NATION",
                "计划出栏量": "GL_M_PLANNED_OUTPUT_NATION",
                "销量环比": "GL_M_SALES_MOM_NATION",
                "存栏数": "GL_M_INVENTORY_NATION",
                "出生数环比": "GL_M_BIRTH_MOM_NATION",
            }
            
            # 匹配指标类型
            metric_key = None
            metric_name = None
            for key, value in metric_key_map.items():
                if key in indicator_type:
                    metric_key = value
                    metric_name = indicator_type
                    break
            
            # 如果没有匹配到，使用默认命名
            if not metric_key:
                # 生成metric_key：将指标类型转换为代码格式
                metric_key = f"GL_M_{indicator_type.upper().replace(' ', '_')}"
                metric_name = indicator_type
            
            # 如果指标名称包含更多信息，添加到metric_key中
            if len(parts) > 2:
                # 添加第一部分（如"商品猪"、"能繁殖母猪"等）
                first_part = parts[0].strip()
                if first_part and first_part != "猪":
                    metric_key = f"GL_M_{first_part}_{indicator_type}".upper().replace(' ', '_').replace('：', '_')
                    metric_name = f"{first_part}：{indicator_type}"
            
            print(f"处理指标: {metric_name} (列 {col_idx}, key: {metric_key})")
            ...
            metric = self._get_or_create_metric(
                ...
                source_code="GANGLIAN",
                sheet_name=sheet_name,
                unit=units[col_idx] if col_idx < len(units) else None,
                raw_header=raw_header_str,
                freq="M"  # 月度数据
            )
            ...
            for row_idx, row in df.iterrows():
                try:
                    ...
                    # 构建dedup_key
                    tags = {"scope": "nation", "source": "GANGLIAN", "indicator_type": indicator_type}
                    ...
                    print(f"处理{metric_name}第{row_idx}行数据失败: {e}")
                    continue
            
            self.db.flush()
            total_inserted += inserted_count
            indicator_results[metric_name] = inserted_count
            print(f"  {metric_name}: 插入 {inserted_count} 条数据")
        
        log_entry = {
            "sheet": sheet_name,
            "metric": "月度数据（多指标）",
            "metric_key": "GL_M_MONTHLY_DATA",
            "inserted": total_inserted,
        ...
        return {"inserted": total_inserted, "indicators": indicator_results, "log": log_entry}
    ...
    def _get_or_create_metric(
        ...
        if not metric:
            # 根据sheet名称和指标名称推断频率
            if freq == "D":
                if "周" in raw_header or "周度" in sheet_name or "（周）" in raw_header:
                    freq = "W"
    ...
    def _save_extraction_log(self, results: Dict):
        ...
        log_data = {
            ...
            "results": {
                ...
                "frozen_capacity_rate": results["frozen_capacity_rate"].get("log", {}),
                "slaughter_weight": results["slaughter_weight"].get("log", {}),
                "monthly_data": results["monthly_data"].get("log", {})
            },
            "total_inserted": (
                ...
                results["frozen_capacity_rate"].get("inserted", 0) +
                results["slaughter_weight"].get("inserted", 0) +
                results["monthly_data"].get("inserted", 0)
            )
        }
...
def main():
    ...
    try:
        ...
        # 冻品库容率统计
        frozen_result = results['frozen_capacity_rate']
        print(f"\n冻品库容率: 插入 {frozen_result.get('inserted', 0)} 条")
        
        # 宰后均重统计
        weight_result = results['slaughter_weight']
        print(f"\n宰后均重（分省区）: 总计插入 {weight_result.get('inserted', 0)} 条")
        if 'provinces' in weight_result:
            print("  各省区详情:")
            for province, count in weight_result['provinces'].items():
                print(f"    {province}: {count} 条")
        
        # 月度数据统计
        monthly_result = results['monthly_data']
        print(f"\n月度数据（多指标）: 总计插入 {monthly_result.get('inserted', 0)} 条")
        if 'indicators' in monthly_result:
            print("  各指标详情:")
