    ...
    try:
        ...
        for sheet in raw_sheets:
            ...
            if raw_table:
                ...
                print(f"  - {sheet.sheet_name} (有数据, {row_count}行)")
            else:
                print(f"  - {sheet.sheet_name} (无数据)")
        ...
        # 验证导入的数据
        print("\n验证导入的数据:")
        target_sheet = db.query(RawSheet).filter(
            RawSheet.raw_file_id == raw_file.id,
            RawSheet.sheet_name == "期货结算价(1月交割连续)_生猪"
        ).first()
        
        if target_sheet:
            target_table = db.query(RawTable).filter(RawTable.raw_sheet_id == target_sheet.id).first()
            if target_table:
                table_data = target_table.table_json
                if isinstance(table_data, str):
                    import json
                    table_data = json.loads(table_data)
                
                print(f"  Sheet名称: {target_sheet.sheet_name}")
                print(f"  数据行数: {len(table_data)}")
                if len(table_data) > 0:
                    print(f"  第1行（表头）: {table_data[0]}")
                    print(f"  第2行（单位）: {table_data[1] if len(table_data) > 1 else 'N/A'}")
                    print(f"  第3行（来源）: {table_data[2] if len(table_data) > 2 else 'N/A'}")
                    print(f"  第4行（数据）: {table_data[3] if len(table_data) > 3 else 'N/A'}")
            else:
                print("  ❌ 未找到对应的raw_table数据")
        else:
            print("  ❌ 未找到目标sheet")
