"""测试钢联日度数据导入功能 - 支持单独测试某个sheet并显示详细调试信息"""
import sys
import os
import io
import json
    ...
    try:
        ...
        if not table_config:
            ...
            if reload == 'y':
                ...
                try:
                    profile_path = project_root / "docs" / "ingest_profile_ganglian_daily_v1.json"
        ...
        if len(observations) == 0:
            ...
            print(f"    2. 日期列名不匹配", flush=True)
            print(f"    3. 数据行格式不符合预期", flush=True)
            print(f"    4. 省区列识别失败", flush=True)
            wb.close()
            return
        ...
        for i, obs in enumerate(observations[:3], 1):
            print(f"    [{i}] metric_key={obs.get('metric_key')}, geo_code={obs.get('geo_code')}, province={obs.get('tags', {}).get('province')}", flush=True)
            ...
            try:
                ...
                if obs_inserted > 0 or obs_updated > 0:
                    ...
                    if sheet_metrics:
                        ...
                        for metric in sheet_metrics[:5]:  # 显示前5个
                            metric_key = None
                            if metric.parse_json:
                                metric_key = metric.parse_json.get("metric_key")
                            province_name = None
                            if metric.raw_header:
                                import re
                                match = re.search(r'：([^：（）]+)（', metric.raw_header)
                                if match:
                                    province_name = match.group(1).strip()
                            print(f"    - {metric.metric_name} ({province_name or '未知省区'}): metric_key={metric_key}", flush=True)
            except Exception as e:
...
def main():
    ...
    print(f"\n{'='*80}", flush=True)
    print(f"钢联日度数据导入测试脚本", flush=True)
    print(f"{'='*80}", flush=True)
    
    # 文件路径
    file_path = project_root / "docs" / "1、价格：钢联自动更新模板.xlsx"
    profile_path = project_root / "docs" / "ingest_profile_ganglian_daily_v1.json"
        ...
        if len(sys.argv) > 1:
            ...
            if sheet_name_arg in wb_temp.sheetnames:
                ...
                if not sheet_name:
                    ...
                    print(f"  可用sheets:", flush=True)
                    for i, name in enumerate(wb_temp.sheetnames, 1):
            ...
            print(f"  2. 列出所有sheets", flush=True)
            print(f"{'='*80}", flush=True)
