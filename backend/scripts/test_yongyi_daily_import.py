"""测试涌益日度数据导入功能 - 支持单独测试某个sheet并显示详细调试信息"""
import sys
import os
import io
import json
    ...
    try:
        ...
        if not table_config:
            ...
            if reload == 'y':
                ...
                try:
                    profile_path = project_root / "docs" / "ingest_profile_yongyi_daily_v1.json"
        ...
        for i, obs in enumerate(observations[:3], 1):
            print(f"    [{i}] metric_key={obs.get('metric_key')}, obs_date={obs.get('obs_date')}, value={obs.get('value')}", flush=True)
            print(f"        tags={json.dumps(obs.get('tags', {}), ensure_ascii=False, indent=8)}", flush=True)
        ...
        # 6.4 保存raw_sheet（简化版，只保存当前sheet）
        raw_sheet = RawSheet(
...
def main():
    ...
    print(f"\n{'='*80}", flush=True)
    print(f"涌益日度数据导入测试脚本", flush=True)
    print(f"{'='*80}", flush=True)
    
    # 文件路径
    file_path = project_root / "docs" / "2026年2月2日涌益咨询日度数据.xlsx"
    profile_path = project_root / "docs" / "ingest_profile_yongyi_daily_v1.json"
    ...
    try:
        ...
        # 加载profile
        print(f"\n🔧 加载导入配置...", flush=True)
        profile = get_profile_by_dataset_type(db, "YONGYI_DAILY")
        if not profile:
