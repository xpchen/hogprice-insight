    ...
    try:
        ...
        try:
            if template_type == "LH_FTR":
                ...
                # 更新batch_id（unified_import会创建新batch）
        ...
        # 更新批次状态
        # 如果有错误，状态应该为failed；如果成功且没有错误，状态为success
        ...
        batch.failed_rows = len(result.get("errors", []))
        ...
        return {
            ...
            "message": "导入完成" if batch.status == "success" else f"导入完成，但有 {len(result.get('errors', []))} 个错误"
        }
