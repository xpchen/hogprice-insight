...
class ColumnMapper:
    ...
    def _extract_value(self, obs: ObservationDict, mapping_config: Dict[str, Any], sheet_config: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if source == "date_col":
            ...
            # 对于PROVINCE_FROM_COLUMN的情况，应该从geo_code获取省份名称
            tags = obs.get("tags", {})
            
            # 检查是否是省份列映射（通过sheet_config判断）
            if sheet_config:
                geo_config = sheet_config.get("geo", {})
                geo_type = geo_config if isinstance(geo_config, str) else geo_config.get("type", "")
                province_cols = sheet_config.get("province_cols", "")
                
                # 如果是PROVINCE_FROM_COLUMN，优先使用geo_code
                if geo_type == "PROVINCE_FROM_COLUMN" and province_cols == "*":
                    geo_code = obs.get("geo_code")
                    if geo_code:
                        # 应用normalizer（如果有）
                        normalizer = mapping_config.get("normalizer")
                        if normalizer and normalizer in self.normalizers:
                            return self.normalizers[normalizer](geo_code)
                        return geo_code
