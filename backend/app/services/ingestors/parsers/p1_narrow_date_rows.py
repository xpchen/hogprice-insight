...
class P1NarrowDateRowsParser(BaseParser):
    ...
    def parse(
        ...
        # 特殊处理：PROVINCE_FROM_COLUMN + province_cols: "*"（如"各省份均价"）
        ...
        if geo_type == "PROVINCE_FROM_COLUMN" and province_cols == "*" and metric_template:
            # 处理宽表格式：日期列 + 多个省份列
            ...
            # 遍历所有列（除了日期列），将每个列名作为省份
            for col_idx, col_name in enumerate(df.columns):
                if col_idx == date_col_idx:
                    continue  # 跳过日期列
                
                province_name = str(col_name).strip()
                if not province_name or province_name == "日期":
                    continue
