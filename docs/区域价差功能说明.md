...
### 2.1 数据来源（权威路径以 docs/0306_生猪 为准）

- **权威文件**：`docs/0306_生猪/生猪/1、价格：钢联自动更新模板.xlsx`（核验数据时请先查看此目录下文件）
- **Sheet**：`区域价差`（区域价差仅来自本 sheet，**不是**「分省区猪价」sheet）
- **数据格式**：列名格式为"商品猪：出栏均价：XX（日） - 商品猪：出栏均价：YY（日）"
- **数据缺失时**：先核对上述 Excel 的「区域价差」sheet 中是否包含该区域对列，勿自行改用其他 sheet 或其它列计算。详见 `docs/数据源核验说明.md`。
...
1. **数据源唯一**：区域价差仅来自钢联「区域价差」sheet；发现某区域对（如广东-重庆）暂无数据时，**先核對** `docs/0306_生猪/生猪/1、价格：钢联自动更新模板.xlsx` 的「区域价差」sheet 是否含对应列，**不要自行修改数据源**（例如不得用分省区猪价两省相减替代）。参见 `docs/数据源核验说明.md`。
2. **单独运行入库**：区域价差功能支持单独运行入库，不影响其他数据提取
3. **标题格式**：指标名称严格按照"区域价差：XX-YY"格式
4. **注释功能**：5日涨跌、10日涨跌通过`/region-spread/changes`接口获取
5. **数据更新**：每次运行提取脚本会更新已有数据，新增数据会插入新记录
6. **去重机制**：使用`dedup_key`确保同一天同一区域对的数据不会重复
